...
> **이후 변경.** nocookie 임베드는 로그인을 넘기지 않으므로 YouTube Premium
> 이용자에게도 광고가 나갑니다. 그래서 재생기 안에 **본인이 직접 켜는**
> 「유튜브 계정」 스위치를 두었습니다 — 켠 사람에 한해 `www.youtube.com`
> 임베드로 바뀌고, 끄면 즉시 되돌아옵니다. 기본값은 여전히 nocookie 이고,
> 선택은 그 사람의 localStorage 에만 남습니다 (privacy.html 제9조에 명시).
...
> **이후 변경 (2026-09-03).** 정확히는 "로그인 없는 임베드로는" 불가능한 것이었습니다.
> 「유튜브 계정」 스위치로 로그인을 임베드에 연결하면 **Premium 계정에 한해**
> 광고가 빠지고, 삼성 인터넷에서는 다른 앱으로 나가도 재생이 이어지는 것을
> 실기기로 확인했습니다 (멎으면 알림의 재생 단추 한 번). Premium 이 아니면
> 여전히 위 설명 그대로이고, 우회가 아니라 YouTube 가 파는 기능을 제값에 쓰는 것이라
> 약관 문제도 없습니다.
