...
모아, **길드 홀 겸 마을 여관**으로 꾸민 재생기에서 틀어주는 프로젝트입니다. 기본 용도는 **야근하며 틀어놓기** — 밤이 깊을수록 홀도 어두워지고 화롯불이 짙어집니다.
...
- **불멍이 되는 불** — 모양이 다른 불꽃 세 장을 장작 앞뒤 다섯 자리에 겹치고, 주기를 서로 안 나누어떨어지는 값(2.3·1.9·1.7·1.3·1.1초)으로 둬서 같은 배열이 한참 동안 돌아오지 않습니다. 불을 안 지핀 시간대에도 잉걸은 숨을 쉽니다. 밤이 깊을수록 아궁이 속이 짙어집니다.
- **맨틀 위의 물건들** — 화로 선반의 등불은 화면 켜두기, 놋쇠 열쇠는 유튜브 계정 연결의 **상태이자 스위치**입니다. 물건을 눌러도 되고 아래 단추를 눌러도 됩니다. 셋째 물건은 직접 찾아보세요.
- **화로에 걸린 음식** — 고기 꼬치, 돼지 통구이, 마법의 냄비, 마시멜로, 감자와 옥수수, 민물고기, 혹은 아무것도. 봉이 있는 것은 봉이 걸이대 가로대에 걸리고, 냄비는 고리로 매달리고, 감자와 옥수수는 잉걸에 묻힙니다. 네 단계로 익다가 안 먹으면 탑니다. **덜 익은 것은 눌러도 안 먹힙니다** — 손만 뻗었다 말고 "아직 이릅니다" 소리가 나며, 다 익은 것은 씹는 소리, 태워 먹은 것은 푸석하게 바스러지는 소리가 납니다. 반응은 화로 위에 바로 뜹니다(휴대폰에서 라온의 말칸이 안 보여도 알 수 있게).
- **합성 소리** — 문간 종, 장작 튀는 소리, 잔 부딪는 소리, 류트 퉁기는 소리, 씹는 소리, 마른 것 바스러지는 소리를 그 자리에서 만듭니다. 음원 파일이 없으니 라이선스도 CSP 구멍도 필요 없습니다.
- **류트 퉁기기** — 라온의 류트를 누르면 문답을 건너뛰고 그 자리에서 곡목을 짜 줍니다. 지금이 몇 시인지를 보고 고릅니다 — 새벽 세 시에 축제 음악을 틀지는 않습니다.
- **여관 야사 11가지** — 홀에서 한 일이 한 줄씩 적힙니다. **아직 못 찾은 것에도 귀띔이 붙어 있어** 어디를 들여다볼지는 알 수 있습니다. 못 찾아도 재생기 쓰는 데 지장은 없습니다.
- **장면 서랍** — 34개 장면을 직접 고르고 여럿 합칠 수 있습니다.
- **곡 내력과 이용 허락 상시 표시** — CC BY 계열은 출처 표기가 이용 조건이므로 항상 띄웁니다. 비상업 조건 곡에는 경고를 붙입니다.
- **유튜브 계정 연결 (선택)** — 기본은 추적 쿠키 없는 `youtube-nocookie.com` 임베드라 프리미엄 계정이어도 광고가 나옵니다. 재생기 안의 「유튜브 계정」 단추를 켜면 로그인이 임베드로 넘어가 **프리미엄이면 광고가 빠집니다** (시청 기록은 계정에 남습니다 — 그래서 기본 꺼짐). 브라우저에 따라 백그라운드 재생이 되기도 합니다.
...
음유시인 문답의 "노래는 어느 만큼 들리면 되겠습니까" 답변이 이 하한을 정합니다.
...
그림도 같은 정신으로 재현할 수 있게 해 두었습니다.

```
tools/gen_hearth_art.mjs   화로 그림 뽑기 (gpt-image-2, 프롬프트가 곧 기록)
tools/make_hearth_art.mjs  뽑힌 그림 다듬기 (배경 알파 정리·크기) → assets/art/
tools/make_bard_art.mjs    라온 넉 장 다듬기 (배경 파내기·얼굴 정렬)
```

자세한 규격과 요령은 [`assets/art/README.md`](assets/art/README.md) 에 있습니다.
...
assets/js/character.js  음유시인 그림 (그린 SVG / 그림 넉 장 / 영상 중에 고름)
...
`character.js` 의 `ART.source` 를 `'svg'` 로 돌리면 예전에 그려 둔 벡터 음유시인으로 돌아갑니다
